...
// ⚠️ Гучність на РЕАЛЬНОМУ пасивному п'єзо. Чистий синус — «правильний» тон,
// але помітно ТИХІШИЙ за меандр при однаковій цифровій амплітуді: п'єзо —
// механічний резонатор, і його збуджує саме ШВИДКА ЗМІНА напруги (dV/dt), а
// синус на звукових частотах змінюється значно повільніше меандра. Тому
// нижче — не чистий синус, а «квазі-меандр»: класичний зрізаний ряд Фур'є
// (1-а + 1/3 3-ої + 1/5 5-ої гармоніки), той самий принцип, яким будують
// square-wave з синусоїд. Це дає СУТТЄВО більше «крутих» фронтів (а з ними —
// гучності на п'єзо), лишаючись при цьому плавним, безрозривним сигналом —
// не голим меандром, який і був причиною різкості до переходу на ЦАП.
// Портаменто/огинаюча фрази (buzzTask/buzzPlay) цю форму хвилі не зачіпають:
// вони й далі ведуть плавний перехід МІЖ нотами, тут — лише форма ВСЕРЕДИНІ
// ноти.
static const int8_t BZ_WAVE[256] = {
    0,   10,   20,   30,   39,   49,   58,   66,   74,   82,   89,   96,  102,  107,  112,  116,
  119,  122,  124,  126,  127,  127,  127,  126,  125,  124,  122,  121,  118,  116,  114,  111,
  109,  107,  105,  102,  101,   99,   97,   96,   95,   95,   94,   94,   95,   95,   96,   97,
   98,   99,  101,  102,  104,  106,  108,  109,  111,  112,  114,  115,  116,  117,  117,  118,
  118,  118,  117,  117,  116,  115,  114,  112,  111,  109,  108,  106,  104,  102,  101,   99,
   98,   97,   96,   95,   95,   94,   94,   95,   95,   96,   97,   99,  101,  102,  105,  107,
  109,  111,  114,  116,  118,  121,  122,  124,  125,  126,  127,  127,  127,  126,  124,  122,
  119,  116,  112,  107,  102,   96,   89,   82,   74,   66,   58,   49,   39,   30,   20,   10,
    0,  -10,  -20,  -30,  -39,  -49,  -58,  -66,  -74,  -82,  -89,  -96, -102, -107, -112, -116,
 -119, -122, -124, -126, -127, -127, -127, -126, -125, -124, -122, -121, -118, -116, -114, -111,
 -109, -107, -105, -102, -101,  -99,  -97,  -96,  -95,  -95,  -94,  -94,  -95,  -95,  -96,  -97,
  -98,  -99, -101, -102, -104, -106, -108, -109, -111, -112, -114, -115, -116, -117, -117, -118,
...
static void IRAM_ATTR buzzIsr() {
    ...
    int16_t out = 128 + (int16_t)(((int32_t)BZ_WAVE[idx] * g_bzAmp) >> 8);
    if (out < 0) out = 0;
    if (out > 255) out = 255;
