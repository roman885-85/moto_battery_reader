...
// Анімація батареї на головній сторінці (пульсація заповнення). ТИПОВО ВИМКНЕНА:
// на деяких панелях ST7789 періодичні часткові перемальовки в loop() заважали
// нормальній роботі дисплея, тож анімація тепер ДОБРОВІЛЬНА. Щоб УВІМКНУТИ —
// розкоментуйте (решта дисплея працює однаково в обох випадках):
// #define ENABLE_BATTERY_ANIM
