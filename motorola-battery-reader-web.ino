...
void setup() {
    ...
    // ⚑ ГОТОВА ПЛАТА DC/DC НА TL494: ТУТ ЦЕ НАЙВАЖЛИВІШИЙ РЯДОК УСЬОГО setup().
    //  Плата пропускає вхід на вихід (~14 В) БЕЗ жодного керування — єдине, що
    //  стоїть між пакетом і цією напругою, — enable. Поки пін висить входом,
    //  рівень на ньому невизначений, і після скидання сторожем посеред заряду
    //  каскад цілком може лишитись увімкненим.
    //  Тому опускаємо його ТУТ, а не в chargeInit(): той кличеться на сотню
    //  рядків нижче, після дисплея, SPI й Wi-Fi — тобто після всього, що вміє
    //  зайняти секунди або впасти. chargeInit() зробить те саме ще раз, і це
    //  не дублювання: там воно частина штатної послідовності «уставка в нуль,
    //  потім enable», а тут — аварійне гасіння до будь-якої ініціалізації.
    ...
    //
    // ⚠️ ПОРЯДОК ТУТ ЗНАЧУЩИЙ: displayInit() мусить іти РАНІШЕ за
    // dischargeInit(). Причина неочевидна — SPI.begin() без аргументів чіпляє
    // не лише SCK/MOSI, а й MISO, а для VSPI це GPIO19, тобто рівно той пін,
    // на якому в цьому проєкті сидить затвор ключа розряду (LOAD_PIN).
    // Дисплею читання не потрібне взагалі, але бібліотека про це не знає.
    // Працює воно тому, що пізніший ledcAttach() у dischargeInit()
    // перепризначає пін через матрицю GPIO й перекриває MISO, а SPI.begin()
    // кличеться один раз і назад уже не відіграє.
    // Переставите виклики місцями — розряд МОВЧКИ перестане керуватись:
    // прошивка збереться, ключ просто не відкриється.
    displayInit();
    displayButtonSetup();
    ...
    // ⚑ ПАРОЛЬ ЗА ЗАМОВЧУВАННЯМ + РАДІО = поганий збіг. По USB це не мало
    //  значення (перепустка — кабель), по Bluetooth має: у радіусі дії
    //  опиняється будь-хто, а пароль зі стандартної збірки відомий усім.
    //  Не блокуємо збірку — це вибір власника, — але мовчати теж не можна.
    if (String(ADMIN_PASSWORD) == "admin123")
        Serial.println("⚠ BT: ADMIN_PASSWORD стандартний («admin123»). По радіо це майже "
                       "те саме, що без пароля — змініть його в settings.h.");
#endif
...
void loop() {
    ...
    // Контроль живлення +14 В У СПОКОЇ. Під час заряду живлення міряє сам
    // chargeTask() (на закритому ключі, разом із напругою пакета), а тут — коли
    // заряд не йде: несправний блок треба показати ДО того, як користувач
    // натисне «заряд», а не після невдалого старту. Індикація постійна
    // (LED_FAULT — два червоних спалахи з паузою), тож помилку видно й без
    // екрана та веба.
    chargePsuIdleTask();

    // Блимання напису на сторінці помилки живлення. Окремим завданням, а не
    // всередині chargePsuIdleTask(), бо періоди різні: живлення міряємо раз на
    // 2 с, а блимати треба двічі на секунду. Поза сторінкою помилки функція
    // виходить одразу.
    displayPsuBlinkTask();
    ...
    // ⚑ ОДИН КВАНТ ЧАСУ — БЕЗДІЯЛЬНІЙ ЗАДАЧІ FreeRTOS. НЕ ПРИБИРАТИ.
    //  Увесь цикл вище — суцільне неблокуюче опитування: жодного delay(),
    //  жодного очікування. Це добре для чутливості кнопок, але має наслідок,
    //  про який легко не здогадатись: задача loop() має пріоритет 1 і прибита
    //  до ядра 1, а бездіяльна задача IDLE1 має пріоритет 0. Поки цикл не
    //  блокується ЖОДНОГО разу, IDLE1 не отримує процесор узагалі.
    //
    //  Чим це погано:
    //   • саме IDLE звільняє пам'ять видалених задач FreeRTOS — без неї купа
    //     повільно тане, а це вже інший клас «раптових перезавантажень»;
    //   • будь-яке налаштування Task WDT, що стежить за IDLE1, гарантовано
    //     спрацює — рівно це й було причиною перезавантажень під час заряду
    //     та розряду (докладно у wdt.h).
    //
    //  delay(1) — це vTaskDelay(1 такт): цикл засинає на одну мілісекунду й
    //  віддає ядро. Для опитування кнопок, анімації (~9 к/с) і USB (11 байт/мс
    //  проти 256-байтного буфера) ця міліcекунда непомітна.
    delay(1);
}
