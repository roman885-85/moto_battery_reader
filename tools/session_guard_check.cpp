...
int main() {
    ...
    // ── 45. ПРИХОВАНІ ЖЕСТИ: УТРИМАННЯ ТА ПОСЛІДОВНІСТЬ ────────────────────
    //  Кнопок три, і всі три зайняті навігацією. Вимикач заряду живе на
    //  УТРИМАННІ, приховане повідомлення — на ПОСЛІДОВНОСТІ. Це треба
    ...
    printf("\n45) жести: сходинки не плутаються, ланцюжок не набирається сам\n");
    {
        // ⚑ ВІДСТАНЬ МІЖ СХОДИНКАМИ. Число задав власник (5 с), але
        //  перевіряємо не його, а те, що між сходинками можна ЗУПИНИТИСЬ:
        //  звичайне довге натискання мусить закінчитись задовго до вимикача.
        ...
        //    операцію: на 0.8 с відкрилось би меню, і аж потім заряд.
        {
            Run r; r.press();
        ...
        // г) верхня сходинка тримається скільки завгодно й лишається собою.
        //    Колись вище неї була ще одна (повноекранне повідомлення на 10 с);
        //    її прибрали, і тепер тримати довше просто нема за чим.
        {
            Run r; r.press(); r.hold(COMBO_LONG_MS);
            r.hold(COMBO_HOLD_CHARGE_MS - COMBO_LONG_MS);
            check(r.hold(20000) == CHOLD_NONE, "понад 5 с нових сходинок немає");
            check(r.release(1) == CHOLD_CHARGE, "…і відпускання досі означає заряд");
        }
        ...
        // ж) відпущену кнопку не «дотримують».
        {
            Run r;
            for (int i = 0; i < 8; i++) {
        ...
        //  ⚑ ГОЛОВНИЙ РИЗИК ТУТ НЕ «НЕ СПРАЦЮЄ», А «СПРАЦЮЄ САМО». Клавіші
        //  жесту — ті самі, якими гортають меню, і «OK» серед них немає
        //  ЖОДНОГО: увесь жест складається з руху. Повноекранний напис, що
        //  вискакує посеред звичайної роботи, читається як несправність.
        struct Seq {
            ComboSeq s; uint32_t t = 500000;
            bool key(uint8_t k, uint32_t dt = 200) { t += dt; return comboSeqFeed(s, t, k); }
            void tick(uint32_t dt)                 { t += dt; comboSeqTick(s, t); }
        };
        const uint8_t L = CKEY_LEFT, R = CKEY_RIGHT;
        // Той самий ланцюжок, записаний тут ОКРЕМО від COMBO_SEQ: якби тест
        // брав його з того самого масиву, він однаково проходив би на будь-якій
        // зміні — тобто не перевіряв би нічого.
        const uint8_t GEST[] = { L, L, R, R, R, R, L, L, R };
        const int GN = (int)(sizeof(GEST) / sizeof(GEST[0]));

        check(GN == (int)COMBO_SEQ_LEN, "ланцюжок тієї довжини, яку задав власник");
        {
            bool same = true;
            for (int i = 0; i < GN; i++) if (COMBO_SEQ[i] != GEST[i]) same = false;
            check(same, "…і саме той: «‹ ‹ › › › › ‹ ‹ ›»");
            bool noEnter = true;
            for (int i = 0; i < GN; i++) if (COMBO_SEQ[i] == CKEY_ENTER) noEnter = false;
            check(noEnter, "…і «OK» у ньому немає: набрати можна де завгодно, нічого не запустивши");
        }

        // а) правильний ланцюжок спрацьовує — і рівно на останній клавіші.
        {
            Seq q; bool early = false;
            for (int i = 0; i < GN - 1; i++) if (q.key(GEST[i])) early = true;
            check(!early, "перші вісім клавіш мовчать");
            check(q.key(GEST[GN - 1]), "…і жест спрацьовує рівно на дев'ятій");
        }
        {
            //  ⚑ ПІСЛЯ СПРАЦЮВАННЯ — ЧИСТИЙ АРКУШ, А НЕ ВІДКАТ. Хвіст цього
            //  ланцюжка («‹ ‹ ›») є і його початком, тож із відкатом напис
            //  вискочив би вдруге вже за шість клавіш. Двічі поспіль — це
            //  вигляд несправності.
            Seq q;
            for (int i = 0; i < GN; i++) q.key(GEST[i]);
            bool again = false;
            const uint8_t TAIL[] = { R, R, R, L, L, R };   // добір «через хвіст»
            for (uint8_t k : TAIL) if (q.key(k)) again = true;
            check(!again, "одразу після спрацювання напис не вискакує вдруге");
        }

        // б) ВІКНО. Число задав власник: увесь жест — за 5 секунд.
        check(COMBO_SEQ_WINDOW_MS == 5000u, "вікно на весь жест — 5 с");
        {
            Seq q;
            for (int i = 0; i < GN - 1; i++) q.key(GEST[i]);
            check(!q.key(GEST[GN - 1], COMBO_SEQ_WINDOW_MS + 1),
                  "не встиг у вікно — жест не зараховано");
            check(!q.key(GEST[GN - 1]), "…і добрати його однією клавішею неможливо");
        }
        {
            // Дев'ять клавіш за 4.5 с — вкладаємось.
            Seq q; bool fired = false;
            for (int i = 0; i < GN; i++) if (q.key(GEST[i], 500)) fired = true;
            check(fired, "дев'ять клавіш по півсекунди — жест зараховано");
        }
        {
            //  ⚑ ВІКНО ВІД ПЕРШОЇ КЛАВІШІ, А НЕ ВІД ПОПЕРЕДНЬОЇ. Паузи по 700 мс
            //  прийнятні для «допуску темпу», але в сумі виходять за вікно.
            Seq q; bool fired = false;
            for (int i = 0; i < GN; i++) if (q.key(GEST[i], 700)) fired = true;
            check(!fired, "сума пауз теж рахується: вікно на ВЕСЬ жест");
        }
        {
            // Лічильник гасне сам, без жодної клавіші, — так його бачить драйвер.
            Seq q;
            q.key(GEST[0]); q.key(GEST[1]);
            q.tick(COMBO_SEQ_WINDOW_MS + 1);
            bool fired = false;
            for (int i = 2; i < GN; i++) if (q.key(GEST[i])) fired = true;
            check(!fired, "лічильник гасне сам собою, а не «на наступному натисканні»");
        }
        {
            //  ⚑ ОБМОВКА, ПІСЛЯ ЯКОЇ ЩОСЬ ЛИШИЛОСЬ ЗАРАХОВАНИМ, НЕ ДАРУЄ НОВИХ
            //  П'ЯТИ СЕКУНД. Ті клавіші натиснуто РАНІШЕ, і відлік для них уже
            //  йде; інакше жест, набраний за півхвилини з помилками, усе одно
            //  зарахувався б.
            //
            //  Набираємо «‹ ‹», доходимо майже до краю вікна, робимо обмовку
            //  (третя «‹»), після якої зараховані лишаються ДВІ, — і добираємо
            //  решту миттєво. Вікно первісного набору вже майже вийшло.
            Seq q;
            q.key(L, 100); q.key(L, 100);         // n = 2, відлік пішов
            q.key(L, 4700);                       // обмовка: лишається 2, час той самий
            bool fired = false;
            const uint8_t REST[] = { R, R, R, R, L, L, R };
            for (uint8_t k : REST) if (q.key(k, 100)) fired = true;
            check(!fired, "обмовка не продовжує вікно первісного набору");

            //  А от повний ПРОВАЛ (нічого не лишилось) — це вже новий набір, і
            //  він законно дістає своє вікно.
            Seq q2;
            q2.key(L, 100);
            q2.key(CKEY_ENTER, 4800);             // не лишає нічого
            bool ok2 = false;
            for (int i = 0; i < GN; i++) if (q2.key(GEST[i], 200)) ok2 = true;
            check(ok2, "…а набір з нуля дістає своє вікно, а не чужий залишок");
        }

        // в) ⚑ ВІДКАТ ПІСЛЯ ОБМОВКИ — І САМЕ ТОЙ, ЯКИЙ ПОТРІБЕН ЦЬОМУ
        //    ЛАНЦЮЖКУ. Спрощений («почати заново цією клавішею») достатній був
        //    для попереднього жесту й НЕ достатній для цього: «‹ ‹» наприкінці
        //    є його власним початком.
        check(comboSeqAfterMiss(8, CKEY_LEFT) == 2,
              "після восьми збігів зайва «‹» лишає ДВІ зараховані, а не одну");
        check(comboSeqAfterMiss(2, CKEY_LEFT) == 2, "третя «‹» поспіль лишає дві");
        check(comboSeqAfterMiss(6, CKEY_RIGHT) == 0, "зайва «›» після шести не лишає нічого");
        check(comboSeqAfterMiss(4, CKEY_LEFT) == 1, "«‹» посеред пробігу «›» стає новим початком");
        {
            //  І те саме поведінкою, а не числом: набір із зайвою «‹» на
            //  восьмій клавіші мусить добратись рештою ланцюжка.
            Seq q; bool fired = false;
            const uint8_t SLIP[] = { L, L, R, R, R, R, L, L,   L,
                                     R, R, R, R, L, L, R };
            for (uint8_t k : SLIP) if (q.key(k, 150)) fired = true;
            check(fired, "обмовка посеред набору не змушує починати з нуля");
        }
        {
            Seq q; bool fired = false;
            const uint8_t SLIP2[] = { L, L, L, R, R, R, R, L, L, R };   // зайва на початку
            for (uint8_t k : SLIP2) if (q.key(k, 150)) fired = true;
            check(fired, "зайва «‹» на початку теж не ламає жест");
        }

        // г) ⚑ ЖЕСТ НЕ НАБИРАЄТЬСЯ ЗВИЧАЙНИМ ГОРТАННЯМ. Перебираємо те, що
        //    людина справді робить: довгі пробіги в один бік і рівне
        //    чергування. Жодне з цього не сміє показати напис.
        {
            struct { uint8_t a, b; int la, lb; const char *what; } DAY[] = {
                { CKEY_RIGHT, CKEY_RIGHT, 1, 1, "гортання лише вперед" },
                { CKEY_LEFT,  CKEY_LEFT,  1, 1, "гортання лише назад" },
                { CKEY_LEFT,  CKEY_RIGHT, 1, 1, "рівне «туди-сюди»" },
                { CKEY_LEFT,  CKEY_RIGHT, 3, 3, "пробіги по три" },
                { CKEY_LEFT,  CKEY_RIGHT, 5, 5, "пробіги по п'ять" },
            };
            for (auto &d : DAY) {
                Seq q; bool fired = false;
                for (int rep = 0; rep < 60; rep++) {
                    for (int i = 0; i < d.la; i++) if (q.key(d.a, 250)) fired = true;
                    for (int i = 0; i < d.lb; i++) if (q.key(d.b, 250)) fired = true;
                }
                char m[130];
                snprintf(m, sizeof(m), "%s — жест не спрацьовує сам", d.what);
                check(!fired, m);
            }
        }
        {
            //  ⚑ І ЧЕСНЕ ЧИСЛО ПРО ВИПАДКОВЕ СПРАЦЮВАННЯ. У жесті лише дві
            //  різні клавіші, тож випадковий набір рано чи пізно його дасть —
            //  питання лише, наскільки рідко. Міряємо, а не сподіваємось.
            Seq q; int fired = 0;
            uint32_t rnd = 2024;
            for (int i = 0; i < 20000; i++) {
                rnd = rnd * 1103515245u + 12345u;
                if (q.key(((rnd >> 20) & 1) ? CKEY_RIGHT : CKEY_LEFT, 300)) fired++;
            }
            char m[150];
            snprintf(m, sizeof(m), "20000 випадкових «‹/›» дали жест %d раз(и) "
                                   "— тобто раз на ~%d натискань", fired, fired ? 20000 / fired : 20000);
            check(fired <= 60, m);
        }

        // д) скидання забирає лічильник цілком.
        {
            Seq q;
            for (int i = 0; i < GN - 1; i++) q.key(GEST[i]);
            comboSeqReset(q.s);
            check(!q.key(GEST[GN - 1]), "після скидання жест не добирається однією клавішею");
        }

        // е) «нема клавіші» — це не клавіша.
        {
            Seq q;
            check(!q.key(CKEY_NONE) && !q.key(CKEY_NONE), "порожня подія нічого не рахує");
            bool fired = false;
            for (int i = 0; i < GN; i++) {
                q.key(CKEY_NONE);
                if (q.key(GEST[i])) fired = true;
            }
            check(fired, "…і не заважає справжньому набору");
        }
    ...
    printf("\n47) вимикач заряду ввімкнено в роботу: кнопки, меню, API, клієнти\n");
    {
        ...
        for (const char *d : { "display.h", "display_color.h" }) {
            ...
            check(fileCalls(d, "hev == CHOLD_CHARGE"),        "…сходинка заряду доходить до дії");
            // ⚑ ПРИХОВАНИЙ ЖЕСТ ПЕРЕЇХАВ З УТРИМАННЯ НА ПОСЛІДОВНІСТЬ. Тут
            //  перевіряється не сама послідовність (її поведінку перебрано в
            //  розділі 45), а те, що драйвер її СПРАВДІ ПИТАЄ — і питає на
            //  кожному проході, інакше лічильник не гаснув би сам.
            check(fileHasNo(d, "CHOLD_FLASH"),               "…а сходинки, якої більше немає, не лишилось");
            check(fileCalls(d, "comboSeqTick(g_seq, millis())") &&
                  fileCalls(d, "comboSeqFeed(g_seq, millis(), ck)"),
                                                             "…послідовність питається щопроходу");
            check(fileCalls(d, "if (e1 == 2 || e2 == 2 || e3 == 2) comboSeqReset(g_seq)"),
                                                             "…і довге натискання рве ланцюжок");
            // ⚑ КОРОТКЕ — ЗЧИТУВАННЯ, ДОВГЕ — МЕНЮ. Правило власника, і воно
            //  живе у ДВОХ драйверах. Поведінку перевіряє display_build_check —
            //  але той збирає лише кольоровий шлях (у монохромного десяток
            //  класів u8g2 на кожну модель екрана, заглушки до них коштували б
            //  більше, ніж дають). Тому для монохромного лишається текст; він
            //  слабший, і саме тому тут перевіряється ОБИДВА боки правила:
            //  що потрібне є І що зворотнього немає.
            check(fileCalls(d, "else if (e3 == 1) { g_readRequested = true"),
                                                             "коротке «OK» на показаннях зчитує пакет");
            check(fileCalls(d, "else if (e3 == 2) { g_displayPage = PAGE_MENU"),
                                                             "…а меню відкриває довге");
            check(fileHasNo(d, "else if (e3 == 1) { g_displayPage = PAGE_MENU"),
                                                             "…і навпаки не лишилось ніде");
            check(fileHasNo(d, "відпустіть = ЗЧИТАТИ"),      "…підказка довгого теж не обіцяє зчитування");
