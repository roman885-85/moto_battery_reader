...
struct RestorePlan {
    ...
    // ⚑ НАРОБІТОК, ЗАПИСАНИЙ У САМОМУ ПАКЕТІ (блок NONSMART, зсуви +3 і +5).
    //  Це ТЕЖ числа донора — такі самі, як його шунт, паливомір і калібрування
    //  нуля АЦП, які панель уже заміняє. Але їх у переліку правок не було, і
    //  еталон приносив у пакет чужий наробіток. Далі зарядна станція чесно
    //  переписувала його в монітор — і це виглядало як «після скидання всі
    //  значення повернулись». Скидання тут ні до чого: їх повертав ЕТАЛОН.
    int      histCcaTpl,  histDcaTpl;    // що несе еталон (донор); -1 — блока немає
    int      histCcaPack, histDcaPack;   // що каже сам пакет;      -1 — блока немає
...
inline void restorePlanBuild(RestorePlan &p, const char *model,
    ...
    // Наробіток, записаний у самих чипах: окремо в еталоні й у пакеті.
    {
        uint16_t a = 0, b = 0;
        p.histCcaTpl  = (tpl33  && impresBmsHistCounters(tpl33,  &a, &b)) ? (int)a : -1;
        p.histDcaTpl  = (p.histCcaTpl  >= 0) ? (int)b : -1;
        p.histCcaPack = (pack33 && impresBmsHistCounters(pack33, &a, &b)) ? (int)a : -1;
        p.histDcaPack = (p.histCcaPack >= 0) ? (int)b : -1;
    }
...
inline void restorePlanRecalc(RestorePlan &p) {
    ...
    if (p.todayY > 0 && useEff > 0) {
        ...
        // ⚑ ДАТА В МАЙБУТНЬОМУ — ЦЕ НЕ «НУЛЬ НАРОБІТКУ», А ЗІПСОВАНА ДАТА.
        //  Раніше вона тихо перетворювалась на нуль (days = 0), і зіпсоване
        //  ставало невідрізненним від законного «пакет запущено сьогодні».
        //  Понад 20 років — теж не дата, а сміття. В обох випадках рахувати
        //  нема з чого, тож etmUseDate лишається нулем: він і є ознака
        //  «порахувалось із придатної дати».
        if (days >= 0 && days * 86400L < (long)RP_ETM_MAX) {
...
inline void restorePlanApply(const RestorePlan &p, uint8_t *d33, uint8_t *d38,
    ...
    // ⚑ НАРОБІТОК ПАКЕТА — ЗАВЖДИ СВІЙ, І ЦЕ НЕ ВИБІР ВЛАСНИКА. CCA/DCA у
    //  блоці NONSMART описують КОНКРЕТНІ банки: скільки в них залито й скільки
    //  взято. Числа донора тут не «інша думка», а просто неправда про цей
    //  пакет — так само, як його шунт чи калібрування нуля АЦП, які панель
    //  заміняє беззастережно. Тому й ця правка беззастережна.
    //
    //  ⚑ І САМЕ ЧЕРЕЗ НЕЇ «ПОВЕРТАЛИСЬ ЛІЧИЛЬНИКИ». Власник обнуляв усе,
    //  писав еталон — і еталон тихо приносив наробіток донора. Зарядна станція
    //  далі робила те, що робить завжди: переписувала його з пакета в монітор.
    //  Виглядало як «скидання не працює», хоча скидання відпрацювало чесно.
    //
    //  Немає що взяти з пакета (чистий чи стертий чип) — пишемо нулі: у
    //  свіжого чипа наробітку немає, а донорського тут бути не сміє.
    if (d33) {
        uint16_t cca = (p.histCcaPack >= 0) ? (uint16_t)p.histCcaPack : 0;
        uint16_t dca = (p.histDcaPack >= 0) ? (uint16_t)p.histDcaPack : 0;
        impresHistCountersWrite(d33, cca, dca);
    }
}

// ═══════ УЗГОДИТИ НАРОБІТОК ІЗ ДАТОЮ, ЗАПИСАНОЮ В ПАКЕТІ ══════════════════
//  Рація не питає DS2433 про «дату першого користування» — вона рахує її як
//  «зараз мінус наробіток» із DS2438. Тому пакет, який щойно повернувся зі
//  станції, показує чужу дату: ЗП переписує наробіток своїм числом, і воно
//  більше за вік самого пакета.
//
//  Узгоджений наробіток обчислюється однозначно: скільки секунд минуло від
//  дати першого запуску, записаної в самому пакеті, до сьогодні. Саме це вже
//  рахує restorePlanRecalc() (p.etmCalc) — тут лише вирок, чи можна й чи варто
//  його застосувати. Виносимо окремо, бо це рішення, а не арифметика: його
//  перевіряє хостовий тест, а Майстер лише виконує.
//
//  ⚑ ПІДІЙМАТИ НАРОБІТОК НЕ БЕРЕМОСЯ. Біда, заради якої крок існує, — наробіток
//  БІЛЬШИЙ за вік. Якщо він менший, це не поломка: пакет міг просто мало
//  працювати, і «дорахувати» йому зношення означало б вигадати роботу, якої не
//  було. Такий випадок лишається людині на сторінці синхронізації.
//
//  ⚑ ВІДМОВА НАЗИВАЄ СВОЇ ЧИСЛА. Перша редакція казала лише «чому саме» —
//  словами. Цього виявилось мало: власник двічі приходив зі скаргою «крок
//  каже, що узгодити неможливо», і з самого тексту не було видно, ЯКУ дату
//  прилад узяв за сьогодні, яку прочитав у пакеті й скільки діб побачив у
//  моніторі. Кожен такий обмін коштував доби. Тепер числа в тексті, і
//  повідомлення розбирає себе саме.
inline bool restoreEtmFixPlan(const RestorePlan &p, uint32_t *etmOut,
                              char *why, size_t whyN) {
    if (why && whyN) why[0] = '\0';
    auto no = [&](const char *m) { if (why && whyN) snprintf(why, whyN, "%s", m); return false; };
    int uy = p.etmUseDate / 10000, um = (p.etmUseDate / 100) % 100, ud = p.etmUseDate % 100;
    long calcD = p.etmCalc / 86400L, packD = p.etmPack / 86400L;

    // ⚑ СПЕРШУ РЕЗУЛЬТАТ, ПОТІМ ПРИЧИНА. Спокуса перевіряти вхідні дані по
    //  черзі — «є сьогодні?», «є дата?», «порахувалось?» — дає дві МЕРТВІ
    //  умови: restorePlanRecalc() і так лишає etmUseDate нулем, коли бракує
    //  будь-чого з них, тож до окремих перевірок справа не доходить. Звірка
    //  від протилежного це й показала: обидві можна прибрати, і жоден тест не
    //  впаде. Тому питаємо про результат, а розбір «чому саме» робимо ВСЕРЕДИНІ
    //  відмови — там кожна гілка досяжна й кожна дає свій текст.
    //
    //  ⚑ ПИТАЄМО etmUseDate, А НЕ etmCalc. Тут була справжня біда: «нуль
    //  наробітку» — це ЗАКОННИЙ результат, а не невдача. Пакет, якому щойно
    //  зробили скидання й поставили сьогоднішню дату першого запуску, має
    //  рівно нуль діб віку — і саме він потребує кроку найбільше, бо в
    //  моніторі після станції лежать тисячі діб. Стара умова `etmCalc <= 0`
    //  плутала «не порахувалось» із «порахувалось, і вийшов нуль», і крок
    //  відмовляв рівно тому пакетові, заради якого писався. Поле etmUseDate
    //  для цього й заведене — див. коментар у restorePlanRecalc().
    if (!p.etmUseDate) {
        if (p.todayY <= 0)
            return no("пристрій не знає сьогоднішньої дати — порахувати наробіток нема від чого");
        if (p.useReal <= 0 && p.useUserY <= 0)
            return no("у пакеті немає дати першого запуску — узгоджувати наробіток нема з чим");
        return no("дата першого запуску не дає правдоподібного наробітку: "
                  "вона або в майбутньому, або старша за 20 років");
    }
    if (p.etmPack <= 0) {
        if (why && whyN) snprintf(why, whyN,
            "наробітку в моніторі й так немає — правити нема чого "
            "(за датою пакета %04d-%02d-%02d мало б бути %ld діб)", uy, um, ud, calcD);
        return false;
    }
    // Доба допуску: наробіток і дата міряються в різних одиницях, і різниця в
    // кілька годин — не розходження, а округлення.
    if (p.etmPack <= p.etmCalc + 86400L) {
        if (why && whyN) snprintf(why, whyN,
            "наробіток уже узгоджений із датою пакета: у моніторі %ld діб, "
            "за датою %04d-%02d-%02d — %ld діб", packD, uy, um, ud, calcD);
        return false;
    }
    if (etmOut) *etmOut = (uint32_t)p.etmCalc;
    return true;
}
