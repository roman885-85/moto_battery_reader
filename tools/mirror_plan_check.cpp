...
#include "templates.h"          // RestorePlan тягне за собою еталони
#include "restore_plan.h"       // вирок «чи узгоджувати наробіток із датою»
...
int main() {
    ...
    // приписує йому чужу особу — і робить це «за планом», тобто впевнено.
    {
        ...
        mirrorPlanSetEtm(p, 6397, -1);
        check(!p.haveAge && !p.etmForeign,
              "без дати не звинувачуємо: нема з чим порівнювати — нема й підозри");

        // ⚑ НУЛЬ ДІБ — ЦЕ ВІК, А НЕ «НЕВІДОМО». Пакет, якому щойно вписали
        //  сьогоднішню дату виготовлення (а так і роблять під час ремонту),
        //  існує рівно нуль діб — і тисячі діб у моніторі проти нього це
        //  найгрубіше розходження, яке взагалі буває. Стара умова `age > 0`
        //  мовчки записувала такий пакет у «нема з чим порівнювати», через
        //  що Майстер не пропонував кроку там, де картка даних уже
        //  попереджала: клієнти рахують це правило без тієї умови.
        mirrorPlanSetEtm(p, 6397, 0);
        check(p.haveAge && p.etmForeign,
              "пакет, зроблений СЬОГОДНІ, з тисячами діб у моніторі — підозра");
        mirrorPlanSetEtm(p, 10, 0);
        check(p.haveAge && !p.etmForeign,
              "…але допуск і тут працює: десяток діб — не звинувачення");
    ...
    // ── УЗГОДЖЕННЯ НАРОБІТКУ З ДАТОЮ ПАКЕТА ────────────────────────────────
    //  Скарга власника: «Напрацювання ETM (3859 діб) більше за вік пакета
    //  (1 доба)». Так і буває після станції: ЗП переписує наробіток своїм
    //  числом, а рація показує «дату першого користування» як «зараз мінус
    //  наробіток» — тобто чужу. Правильне число виводиться однозначно з дати
    //  першого запуску, записаної в самому пакеті. Тут — вирок, чи можна й чи
    //  варто його застосувати.
    printf("\nX) наробіток узгоджується з датою, записаною в пакеті\n");
    {
        RestorePlan p;
        memset(&p, 0, sizeof(p));
        uint32_t etm = 0; char why[176] = "";

        // ⚑ Перевіряємо не лише ВІДМОВУ, а й ПРИЧИНУ. Сама відмова тут
        //  однакова для трьох різних станів, і тест, що дивиться лише на неї,
        //  не бачить, чи розбір причин узагалі працює.
        check(!restoreEtmFixPlan(p, &etm, why, sizeof(why)) && strstr(why, "сьогоднішньої дати"),
              "без сьогоднішньої дати крок відмовляється — і каже саме це");
        p.todayY = 2026; p.todayM = 8; p.todayD = 21;
        check(!restoreEtmFixPlan(p, &etm, why, sizeof(why)) && strstr(why, "дати першого запуску"),
              "без дати першого запуску — теж відмова, і теж із власною причиною");

        // Пакет запущено 2026-08-20, тобто йому доба. А в моніторі 3859 діб.
        p.useReal    = 20260820;
        p.etmUseDate = 20260820;                       // ознака «порахувалось»
        p.etmCalc    = 1L * 86400L;
        p.etmPack    = 3859L * 86400L;
        check(restoreEtmFixPlan(p, &etm, why, sizeof(why)), "розходження бачить і береться правити");
        check(etm == 86400u, "…і ставить рівно стільки, скільки минуло від дати пакета");

        // ⚑ НУЛЬ ДІБ — ЗАКОННИЙ РЕЗУЛЬТАТ, А НЕ НЕВДАЧА. Пакет, якому щойно
        ...
        //  моніторі після станції лежать тисячі діб. Стара умова питала
        //  `etmCalc <= 0` і відмовляла рівно цьому пакетові; власник це й
        //  побачив як «крок каже, що узгодити неможливо».
        p.useReal = p.etmUseDate = 20260821;           // запущено сьогодні
        p.etmCalc = 0;
        check(restoreEtmFixPlan(p, &etm, why, sizeof(why)),
              "пакет, запущений СЬОГОДНІ, крок узгоджує, а не відмовляє");
        check(etm == 0u, "…і ставить нуль, бо стільки йому й є");

        // ⚑ ПІДІЙМАТИ НАРОБІТОК НЕ БЕРЕМОСЯ: менший за вік — це не поломка, і
        //  дораховувати пакетові роботу, якої не було, крок не буде.
        p.useReal = p.etmUseDate = 20260820; p.etmCalc = 1L * 86400L;
        p.etmPack = 0L;
        check(!restoreEtmFixPlan(p, &etm, why, sizeof(why)) && strstr(why, "нема чого"),
              "порожній наробіток крок не «дораховує»");
        check(strstr(why, "2026-08-20") && strstr(why, "1 діб"),
              "…і показує, з яких чисел склався вирок");
        p.etmPack = 12L * 3600L;                       // пів доби проти доби
        check(!restoreEtmFixPlan(p, &etm, why, sizeof(why)), "наробіток МЕНШИЙ за вік крок не чіпає");

        // Доба допуску: різниця в години — округлення, а не розходження.
        p.etmPack = 1L * 86400L + 3600L;
        check(!restoreEtmFixPlan(p, &etm, why, sizeof(why)) && strstr(why, "уже узгоджений"),
              "різниця в години не вважається розходженням");
        check(strstr(why, "у моніторі 1 діб") && strstr(why, "2026-08-20"),
              "…і теж називає обидва числа, а не лише вирок");
        p.etmPack = 3L * 86400L;
        check(restoreEtmFixPlan(p, &etm, why, sizeof(why)), "…а різниця в кілька діб — вважається");

        // Дата, вписана вручну, теж підходить як джерело.
        p.useReal = 0; p.useUserY = 2026; p.useUserM = 8; p.useUserD = 20;
        check(restoreEtmFixPlan(p, &etm, why, sizeof(why)), "вписана вручну дата теж годиться");
        check(why[0] == '\0', "коли крок береться, причини відмови немає");
    }

    // ── ЩО САМЕ ВВАЖАЄТЬСЯ «ПОРАХУВАЛОСЬ» ──────────────────────────────────
    //  Вирок вище питає etmUseDate, тож ціна помилки в ЦЬОМУ полі — рівно та
    //  сама, що й у вироку. Перевіряємо його на живому перерахунку, а не на
    //  вручну виставлених числах: інакше тест звіряв би сам себе.
    printf("\nX) «порахувалось» — це придатна дата, а не будь-яка\n");
    {
        RestorePlan p;
        auto recalc = [&](int uy, int um, int ud) {
            memset(&p, 0, sizeof(p));
            p.todayY = 2026; p.todayM = 8; p.todayD = 21;
            p.useUserY = uy; p.useUserM = um; p.useUserD = ud;
            restorePlanRecalc(p);
        };
        recalc(2026, 8, 20);
        check(p.etmUseDate == 20260820 && p.etmCalc == 86400L, "вчорашня дата дає добу");
        recalc(2026, 8, 21);
        check(p.etmUseDate == 20260821 && p.etmCalc == 0L,
              "сьогоднішня дає нуль — і це ПОРАХУВАЛОСЬ, а не «не змогли»");
        // ⚑ Дата в майбутньому раніше тихо ставала нулем (days = 0), і
        //  зіпсоване робилось невідрізненним від законного «запущено сьогодні».
        recalc(2027, 1, 1);
        check(p.etmUseDate == 0, "дата в майбутньому не вважається порахованою");
        recalc(1990, 1, 1);
        check(p.etmUseDate == 0, "старша за 20 років — теж ні");
    }
