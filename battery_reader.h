...
public:
    ...
    // ── ПЕРЕВІРКА САМОЇ ЛІНІЇ ENABLE/ПІДТЯЖКИ ──────────────────────────────
    // Скарга «читання не працює, підтяжки немає» ніяк не діагностується з
    // прошивки: вона піднімає пін і йде далі, а чи піднялась при цьому ЛІНІЯ —
    // не знає. Тут вона це перевіряє.
    //
    // ⚑ Читання рівня з ВИХІДНОГО піна на ESP32 працює й дає РЕАЛЬНИЙ стан
    // виводу, а не те, що ми туди записали: в arduino-esp32 OUTPUT визначено
    // як 0x03, тобто разом із бітом INPUT, тож вхідний буфер лишається
    // увімкненим. Саме тому зовнішнє коротке замикання чи завелике
    // навантаження тут видно.
    enum { ENL_OK = 0, ENL_STUCK_LOW, ENL_STUCK_HIGH };
    uint8_t enableLineCheck();
    static const char *enableLineText(uint8_t code);

    // ── ПЕРЕВІРКА САМОЇ ШИНИ ДАНИХ ─────────────────────────────────────────
    // enableLineCheck() дивиться на КЕРУЮЧИЙ пін (_pullupPin). Але скарга
    // «немає підтяжки» — про ЛІНІЮ ДАНИХ: підтяжка існує саме для того, щоб
    // підняти _pin, і перевіряти треба результат, а не намір.
    // Логіка: підтяжку вимкнено -> лінія даних мусить бути низькою;
    //         підтяжку ввімкнено -> мусить піднятись.
    // Що ловить: обірваний резистор підтяжки, коротке лінії даних на землю,
    // відсутність спільної землі з пакетом (а саме її й зачіпає шунт у
    // мінусовому проводі, доданий разом із зарядом).
    // ⚠️ ВЕРДИКТИ ПЕРЕПИСАНО. Перша редакція читала «лінія висока без нашої
    // підтяжки» як НЕСПРАВНІСТЬ («зайва зовнішня підтяжка») — і це було
    // помилкою просто задом наперед: саме так виглядає ПРАВИЛЬНА плата зі
    // штатним резистором 2.2…4.7 кОм. На справному пристрої перевірка кричала
    // б про несправність, а на пристрої без резистора мовчала.
    // Тепер: висока в першій фазі = зовнішня підтяжка Є, і це норма.
    enum { BUS_OK = 0,          // зовнішня підтяжка є — штатний стан
           BUS_NO_EXT_PULLUP,   // зовнішньої немає, тримає лише внутрішня
           BUS_NO_PULLUP };     // не піднімає ніхто — обрив або немає землі
