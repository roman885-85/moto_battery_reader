...
inline bool restoreEtmFixPlan(const RestorePlan &p, uint32_t *etmOut, const char **why) {
    ...
    //  умови: restorePlanRecalc() і так лишає etmCalc нулем, коли бракує
    ...
    if (p.etmPack <= 0)
        return no("наробітку в моніторі й так немає — правити нема чого");
    ...
    if (p.etmPack <= p.etmCalc + 86400L)
        return no("наробіток уже узгоджений із датою пакета");
