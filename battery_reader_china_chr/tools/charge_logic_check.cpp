...
static void digitalWrite(int pin, int v);   // визначення — після settings.h (потрібен CHARGE_TL_EN_PIN)
static unsigned long millis() { return 1000; }
// ⚑ ЗАПАМ'ЯТОВУЄМО, ЯКИМ БУВ ENABLE У МИТЬ ПРИКРІПЛЕННЯ ШІМ. На платі TL494
//  це головне питання всього chargeInit(): плата дає повний прохід входу на
//  вихід (~14 В) БЕЗ жодного керування, тож enable мусить лягти в LOW ЩЕ ДО
//  роботи з LEDC. Якщо прикріплення провалиться — а воно вміє провалюватись
//  мовчки, — пакет має лишитись від'єднаним від 14 В.
static bool g_attachSawEnableLow = false;
extern int  g_lastEnable;              // визначено нижче; оголошення тут — для цієї функції
static bool ledcAttachChannel(int, int, int, int) {
    g_attachSawEnableLow = (g_lastEnable == 0);
    return true;
}
static uint32_t g_lastDuty = 0;
// ⚑ ЗАПАМ'ЯТОВУЄМО НЕ ЛИШЕ ЗНАЧЕННЯ, А Й ПОРЯДОК. На платі TL494 порядок
//  «спершу уставка, потім enable» — це запобіжник, а не стиль: у зворотному
//  порядку каскад на мить бачить напругу керування з попереднього сеансу.
//  Перевірити такий порядок можна лише зсередини підмінників, тож прапорець
//  зводиться тут: ledcWrite ставить його, digitalWrite(HIGH) — читає.
int         g_lastEnable = -1;         // -1 = ще не чіпали, 0 = LOW, 1 = HIGH (оголошено вище)
static bool g_ctrlWritten = false;     // уставка йшла після останнього enable?
static bool g_enableAfterCtrl = false; // останній enable HIGH стався ПІСЛЯ уставки
static void ledcWrite(int, uint32_t d) { g_lastDuty = d; g_ctrlWritten = true; }
...
// ⚑ СТЕЖИМО ЗА КОНКРЕТНИМ ПІНОМ, А НЕ ЗА «БУДЬ-ЯКИМ». chargeInit() опускає в
//  LOW і уставку теж, тож підмінник «на всі піни одразу» показував би enable
//  знятим навіть тоді, коли його ніхто не чіпав, — і перевірка порядку
//  мовчала б. Саме на цьому вона й попалась при звірці від протилежного.
static void digitalWrite(int pin, int v) {
#ifdef CHARGE_TL_EN_PIN
    if (pin != CHARGE_TL_EN_PIN) return;
#else
    (void)pin;
#endif
    g_lastEnable = v;
    if (v) g_enableAfterCtrl = g_ctrlWritten;
    g_ctrlWritten = false;
}
...
static int analogRead(int pin) {
    // ⚑ На платі TL494 вимірювальних пінів НЕМАЄ ЖОДНОГО, і цей підмінник там
    //  ніхто не кличе — але зібратись він мусить в обох конфігураціях.
...
int main() {
...
#if CHARGE_IS_BUCK
        // ⚑ ЦЕ ПИТАННЯ ПРО КЛЮЧ, А НЕ ПРО РЕГУЛЯТОР, і воно має сенс лише
        //  там, де ключ наш. На повністю відкритому ключі понижувач
        //  вироджується у пряме з'єднання живлення з пакетом, тож стеля
        //  мусить лишатись СТРОГО нижчою за повну шкалу.
        check(duty < CHARGE_DUTY_FULL,  "ключ НІКОЛИ не відкривається повністю");
#else
        // На платі TL494 шкала — це мілівольти виходу, а її верх — верх
        // калібрувальної таблиці, тобто найбільша напруга, яку плата взагалі
        // видає. Вимагати «строго нижче» тут означало б навмисно не
        // дотягуватись до власної стелі: перевірка стерегла б неіснуючий ключ.
        check(duty == CHARGE_DUTY_FULL, "стеля — це і є верх шкали виходу");
        check(CHARGE_DUTY_FULL == (uint16_t)CHARGE_CAL_OUT_MAX,
              "…а верх шкали — верх калібрувальної таблиці, не відсоток від неї");
#endif
    }
    ...
    // ══ ДАЛІ — ТІЛЬКИ ВЛАСНИЙ ПОНИЖУВАЧ ═══════════════════════════════════
    //  Усе, що нижче, міряє РЕАЛЬНІ деталі: дросель, шунт, подільники, ключ,
    //  блок живлення. На платі TL494 жодної з них немає, тож ці перевірки там
    //  не «пройшли б легше» — вони перевіряли б неіснуюче залізо за
    //  константами, які нічому не відповідають. Замість них — окремий розділ
    //  наприкінці файлу, який питає рівно те, що в ТІЄЇ плати є.
#if CHARGE_IS_BUCK
    ...
    // ══ СВІДОК НАСИЧЕННЯ КЛЕМИ — ТІЛЬКИ ВЛАСНИЙ ПОНИЖУВАЧ ═════════════════
    //  Уся ця машинерія читає ОДНЕ явище: наш подільник упирається в стелю
    //  АЦП, бо на клемі лишилась напруга живлення. На платі TL494 такого
    //  явища не існує — подільника немає, а напругу віддає сам пакет. Там
    //  пакет, що закрився сам, видно інакше: струм падає в нуль, і заряд
    //  завершується за уставкою, а зникнення пакета — за тим, що DS2438
    //  перестав читатись (CHGR_NOREAD).
#if CHARGE_IS_BUCK
    ...
    // ══ ПРОБУДЖЕННЯ — ТІЛЬКИ ВЛАСНИЙ ПОНИЖУВАЧ ════════════════════════════
    //  Режим тримається на ТРЬОХ власних вимірах (струм і напруга на
    //  закритому ключі, струм під час розгону). На платі TL494 усі три
    //  приходять із DS2438 усередині пакета — тобто звідти, звідки
    //  заблокований пакет не віддає нічого. Там режим заборонено цілком
    //  (WAKENO_TOPO), і саме це перевіряє розділ 28.
#if CHARGE_IS_BUCK
    ...
    // ── 26. ВЕРСІЯ ПРИСТРОЮ БЕЗ ЗАРЯДУ ─────────────────────────────────────
    //  Скарга: у цьому екземплярі силової частини немає взагалі, піни CHARGE_*
    //  ні до чого не підключені — і пристрій сумлінно доповідає про несправний
    //  блок живлення, якого в ньому не передбачено.
    //
    //  Вимикач мусить знімати ВСЕ, що спирається на силову частину, а не лише
    //  ховати кнопку: інакше з'явився б стан «заряду немає, але помилка
    //  живлення є» — рівно те, від чого вимикач і рятує.
    printf("\n26) вимикач заряду: знімає й саму функцію, і скарги на живлення\n");
    {
        g_chg = ChargeState{};
        chargeSetOffByUser(false);
        const bool hw = chargeHardware();
        check(hw, "у цій збірці силова частина описана — є що вимикати");
        check(chargeAvailable(), "поки не вимикали — заряд доступний");
        check(chargeUnavailText()[0] == '\0', "…і причини відмови немає");

        // а) вимкнули — зникає і функція, і контроль живлення.
        chargeSetOffByUser(true);
        check(!chargeAvailable(),  "вимкнули — заряду немає");
        check(chargeHardware(),    "…але це не робить вигляд, ніби пінів немає в settings.h");
        check(!chargePsuSensed(),  "контроль живлення теж знято: міряти нічого");
        check(chargePsuState() == PSU_UNKNOWN, "стан живлення — «не знаю», а не старе число");
        check(chargePsuMv() == 0,  "і напруга не показується");
        check(!chargePsuFault(),   "жодної скарги на блок живлення");
        check(!chargePsuScreenActive(), "і сторінка помилки живлення не з'являється");

        // б) причина відмови — саме «версія без заряду», а не «не налаштовано».
        //    Це різні відповіді: перша правиться чотирма натисканнями, друга —
        //    прошивкою. Клієнти показують РІВНО те, що прийшло звідси.
        const char *why = chargeUnavailText();
        check(why[0] != '\0', "причина відмови названа");
        check(strstr(why, "версія") != nullptr,
              "…і вона про версію пристрою, а не про settings.h");
        check(strstr(why, "settings.h") == nullptr,
              "не радимо правити settings.h там, де правити його нема чого");

        // в) СТАРИЙ ВИМІР НЕ ПРОСОЧУЄТЬСЯ. Найтонше місце: до вимикання блок
        //    міг бути класифікований як несправний. Якщо гейт стоїть лише в
        //    місці вимикання, а не в читачів, це число лишиться в пам'яті —
        //    і «несправність» переживе вимикач, бо переміряти вже нікому:
        //    опитування в спокої теж вимкнене.
        g_psuState = PSU_ABSENT; g_psuMv = 3000;
        check(chargePsuState() == PSU_UNKNOWN,
              "підкинутий «несправний блок» крізь вимкнений заряд не проходить");
        // Окремим рядком, бо це ДРУГЕ місце, де гейт можна забрати: вирок про
        // несправність читає стан через ту саму функцію — або, якщо
        // недоглянути, повз неї, просто з g_psuState.
        check(!chargePsuFault(),
              "…і не стає вироком про несправність живлення");
        check(chargePsuPoll() == PSU_UNKNOWN, "опитування живлення не міряє й не судить");

        // г) заряд, що ЙДЕ, вимикач зупиняє. Прапорець сам по собі нічого не
        //    гасить: chargeTask() дивиться на СТАН машини, і без цього пристрій
        //    показував би «заряду немає», тримаючи ключ відкритим.
        chargeSetOffByUser(false);
        g_psuState = PSU_OK; g_psuMv = 14000;
        g_chg = ChargeState{}; g_chg.state = CHG_RUN; g_chg.mode = CHG_MODE_CHARGE;
        check(chargeRunning(), "заряд іде");
        chargeSetOffByUser(true);
        check(!chargeRunning(), "вимикач зупинив заряд, що йшов");
        check(g_chg.duty == 0,  "…і шпаруватість зведено в нуль");

        // д) старт і пробудження відмовляють тією самою причиною.
#if CHARGE_IS_BUCK
        check(chargeWakeRefuse(chargeAvailable(), true, false, PSU_OK, false, 0, 0) == WAKENO_NA,
              "пробудження на версії без заряду не дають");
#else
        // На платі TL494 відмова СИЛЬНІША за вимикач: режиму там немає взагалі,
        // і вмикання заряду назад його не поверне.
        check(chargeWakeRefuse(chargeAvailable(), true, false, PSU_OK, false, 0, 0) == WAKENO_TOPO,
              "пробудження не дають, і причина — залізо, а не вимикач");
#endif

        // е) увімкнули назад — усе повертається, і живлення міряється заново.
        chargeSetOffByUser(false);
#if CHARGE_IS_BUCK
        check(chargeAvailable() && chargePsuSensed(), "увімкнули назад — заряд і контроль живлення повернулись");
#else
        // Контроль живлення тут не «повертається», бо його ніколи й не було:
        // подільника на +14 В у цієї плати немає. Вимикач керує зарядом — і
        // рівно зарядом.
        check(chargeAvailable(),   "увімкнули назад — заряд повернувся");
        check(!chargePsuSensed(),  "…а контроль живлення не з'явився нізвідки");
#endif
        check(chargeUnavailText()[0] == '\0', "…і причина відмови зникла");
        check(chargePsuState() == PSU_UNKNOWN,
              "стан живлення після повернення — «ще не міряли», а не спогад");

        // ж) прапорець просить себе зберегти РІВНО на зміну стану.
        chargeConsumeModeSave();
        chargeSetOffByUser(false);                 // те саме значення — не зміна
        check(!chargeConsumeModeSave(), "повторне вимикання в той самий бік запису не просить");
        chargeSetOffByUser(true);
        check(chargeConsumeModeSave(),  "справжня зміна просить зберегти себе");
        check(!chargeConsumeModeSave(), "…і просить рівно один раз");
        chargeSetOffByUser(false);
        g_chg = ChargeState{};
    }

    // ── 27. ПІДСУМОК ЗНІМАЄТЬСЯ, І ПАНЕЛЬ ПОВЕРТАЄТЬСЯ В ПОЧАТКОВИЙ СТАН ────
    //  Скарга: «після відміни зарядки кнопка початку не повертається, панель
    //  постійно показує попередній режим». Три питання про той самий сеанс
    //  розрізняються прошивкою давно — але клієнти мали лише одне з них, і
    //  плутали «яким показати підсумок» із «чи зайняте залізо».
    printf("\n27) підсумок операції знімається, і режим перестає показуватись\n");
    {
        // а) пробудження ЙДЕ: показуємо його, залізо зайняте.
        g_chg = ChargeState{};
        g_chg.mode = CHG_MODE_WAKE; g_chg.state = CHG_RUN;
        check(chargeWaking(),      "поки пробудження йде — залізо зайняте");
        check(chargeWakeShown(),   "…і панель має вигляд пробудження");

        // б) зупинили: ВИГЛЯД лишається (інакше результат пробудження читався б
        //    під заголовком «ЗАРЯД»), а залізо вже вільне — і саме за цим
        //    питанням клієнти мусять вирішувати, чи ховати кнопку старту.
        chargeStop(CHGR_USER);
        check(!chargeWaking(),     "після зупинки залізо вільне");
        check(chargeWakeShown(),   "…але підсумок і далі підписаний як пробудження");
        check(chargeScreenActive(),"…і сам підсумок нікуди не подівся");

        // в) ⚑ ГОЛОВНЕ: підсумок є чим закрити, і після цього режим перестає
        //    показуватись. Без цієї дороги панель у клієнтах лишалась у
        //    вигляді пробудження назавжди — разом зі схованою кнопкою «ПОЧАТИ
        //    ЗАРЯД», бо ховали її за тим самим прапорцем.
        chargeDismiss();
        check(!chargeScreenActive(), "підсумок знімається");
        check(!chargeWakeShown(),    "…і режим перестає показуватись");
        check(!chargeWaking(),       "…і залізо, звісно, вільне");

        // г) поки операція ЙДЕ, прибрати підсумок не можна: інакше клієнт
        //    погасив би панель посеред роботи.
        g_chg = ChargeState{};
        g_chg.mode = CHG_MODE_CHARGE; g_chg.state = CHG_RUN;
        chargeDismiss();
        check(chargeRunning() && chargeScreenActive(),
              "на операції, що йде, прибирання підсумку не діє");

        // д) те саме для штатного заряду: зупинили — підсумок є, закрили —
        //    панель у початковому стані.
        chargeStop(CHGR_USER);
        check(chargeScreenActive() && !chargeRunning(), "після зупинки заряду лишається підсумок");
        chargeDismiss();
        check(!chargeScreenActive(), "…і він так само знімається");
        g_chg = ChargeState{};
    }

    // ══ 28. ГОТОВА ПЛАТА DC/DC НА TL494 ═══════════════════════════════════
    //  Дзеркало до всього, що вище стоїть під CHARGE_IS_BUCK. Питаємо рівно
    //  те, що в ЦІЄЇ плати є: калібрувальну криву, шкалу в мілівольтах,
    //  enable — і, окремо, ЧЕСНІСТЬ відмов там, де заліза немає.
#if CHARGE_IS_TL494
    printf("\n28) плата TL494: калібрувальна крива й шкала в мілівольтах\n");
    {
        static const uint16_t calCtrl[] = CHARGE_CAL_CTRL_MV;
        static const uint16_t calOut[]  = CHARGE_CAL_OUT_MV;

        // а) сама таблиця мусить бути придатною для інтерполяції. Немонотонна
        //    крива дала б від'ємний знаменник і безглузду напругу керування —
        //    причому мовчки, бо всі числа лишились би в діапазоні uint16_t.
        bool monoOut = true, monoCtrl = true;
        for (int i = 1; i < CHARGE_CAL_POINTS; i++) {
            if (calOut[i]  <= calOut[i - 1])  monoOut  = false;
            if (calCtrl[i] <= calCtrl[i - 1]) monoCtrl = false;
        }
        check(monoOut,  "вихідна напруга в таблиці СТРОГО зростає");
        check(monoCtrl, "напруга керування в таблиці СТРОГО зростає");
        check(calOut[0] == 0, "перша точка — нуль виходу: з неї починається soft-start");
        check(calOut[CHARGE_CAL_POINTS - 1] == (uint16_t)CHARGE_CAL_OUT_MAX,
              "CHARGE_CAL_OUT_MAX — це справді остання точка таблиці, а не окреме число");
        // Напруга керування мусить лишатись у межах, які взагалі можна видати
        // ШІМ+RC від 3.3 В. Вище — це просто недосяжна уставка.
        check(calCtrl[CHARGE_CAL_POINTS - 1] < 3300,
              "верх кривої керування досяжний від 3.3 В");

        // б) інтерполяція влучає у ВУЗЛИ точно — інакше сама таблиця брехала б.
        bool nodesOk = true;
        for (int i = 0; i < CHARGE_CAL_POINTS; i++)
            if (chargeCtrlMvForOutputMv(calOut[i]) != calCtrl[i]) nodesOk = false;
        check(nodesOk, "у вузлах таблиці інтерполяція повертає рівно табличне значення");

        // в) МІЖ вузлами — монотонно й без стрибків назад. Це те, на чому
        //    тримається регулятор: крок угору за виходом мусить бути кроком
        //    угору за керуванням, інакше контур пішов би не в той бік.
        uint16_t prev = 0; bool up = true;
        for (uint32_t o = 0; o <= (uint32_t)CHARGE_CAL_OUT_MAX; o += 10) {
            uint16_t c = chargeCtrlMvForOutputMv((uint16_t)o);
            if (c < prev) up = false;
            prev = c;
        }
        check(up, "крива керування не має ділянок, де більший вихід просить меншої уставки");

        // г) ПОЗА таблицею НЕ екстраполюємо. Вигадана точка тут означала б
        //    напругу, якої ніхто не міряв, — на пакеті це ампери струму.
        check(chargeCtrlMvForOutputMv(0) == calCtrl[0],
              "нижче таблиці — затиск у першу точку");
        check(chargeCtrlMvForOutputMv(65535) == calCtrl[CHARGE_CAL_POINTS - 1],
              "вище таблиці — затиск в останню, а не продовження прямої");

        // ґ) ШКАЛА РЕГУЛЯТОРА — МІЛІВОЛЬТИ ВИХОДУ, і це тотожність, а не оцінка.
        check(chargeDutyForMv(4000) == 4000, "chargeDutyForMv() на цій шкалі — тотожність");
        check(chargeDutyForMv(65535) == (uint16_t)CHARGE_DUTY_MAX, "…затиснута стелею шкали");
        // Стеля пробудження на цій шкалі — це просто сама напруга пробудження.
        check(chargeWakeDutyCapFor(14000) == (uint16_t)CHARGE_WAKE_MV,
              "стеля напруги = напруга, живлення в неї не входить");
    }

    printf("\n28а) плата TL494: що йде в залізо — і в якому порядку\n");
    {
        // а0) ПУСК. Найперша дія chargeInit() — enable у LOW, і саме ДО LEDC:
        //     плата дає повний прохід ~14 В без керування, тож enable — єдиний
        //     запобіжник, і провал прикріплення ШІМ не сміє його скасувати.
        g_lastEnable = -1; g_attachSawEnableLow = false;
        chargeInit();
        check(g_attachSawEnableLow, "enable лежить у LOW ще ДО прикріплення ШІМ");
        check(g_lastEnable == 0,    "…і після chargeInit() лишається знятим");

        g_lastDuty = 0; g_lastEnable = -1;
        chargeResetDutyCap();

        // а) нуль означає «виходу немає»: enable знято, уставка припаркована
        //    в НАЙНИЖЧІЙ точці кривої. Саме припаркована, а не залишена як є:
        //    уставка з попереднього сеансу означала б, що в момент наступного
        //    enable каскад бачить чужу напругу.
        chargeSetDuty(0);
        check(g_lastEnable == 0, "duty 0 -> enable LOW");
        uint32_t raw0 = g_lastDuty;
        uint32_t want0 = (uint32_t)chargeCtrlMvForOutputMv(0) *
                         ((1u << CHARGE_TL_PWM_BITS) - 1u) / 3300u;
        check(raw0 == want0, "…а уставка стоїть у першій точці кривої");

        // б) будь-яке ненульове значення вмикає каскад — і уставка йде ДО
        //    enable. Порядок перевіряємо не за часом, а за наслідком: у
        //    момент, коли enable став HIGH, у LEDC уже лежить нове число.
        //    ⚑ Прапорці зводимо ПЕРЕД самим викликом — інакше запис уставки з
        //    ПОПЕРЕДНЬОГО виклику зарахувався б цьому, і перевірка порядку
        //    лишалась би зеленою навіть на переставленому коді. Саме на цьому
        //    вона й попалась при звірці від протилежного.
        g_ctrlWritten = false; g_enableAfterCtrl = false;
        chargeSetDuty(4000);
        check(g_lastEnable == 1, "ненульова уставка -> enable HIGH");
        uint32_t want4 = (uint32_t)chargeCtrlMvForOutputMv(4000) *
                         ((1u << CHARGE_TL_PWM_BITS) - 1u) / 3300u;
        check(g_lastDuty == want4, "у ШІМ пішла напруга керування з таблиці, а не самі мілівольти");
        check(g_enableAfterCtrl, "уставка виставлена ДО enable, а не після");

        // в) стеля діє в НАЙНИЖЧІЙ точці — так само, як у понижувача.
        chargeSetDuty(65535);
        uint32_t wantMax = (uint32_t)chargeCtrlMvForOutputMv(CHARGE_DUTY_MAX) *
                           ((1u << CHARGE_TL_PWM_BITS) - 1u) / 3300u;
        check(g_lastDuty == wantMax, "прямий виклик повз регулятор затиснуто стелею шкали");

        // г) стеля СЕАНСУ так само діє тут, а не лише в регуляторі.
        chargeSetDutyCap(2000);
        chargeSetDuty(65535);
        uint32_t want2 = (uint32_t)chargeCtrlMvForOutputMv(2000) *
                         ((1u << CHARGE_TL_PWM_BITS) - 1u) / 3300u;
        check(g_lastDuty == want2, "стеля сеансу теж затискає прямий виклик");
        chargeResetDutyCap();

        // ґ) chargeOff() — це саме нуль, тобто знятий enable.
        chargeOff();
        check(g_lastEnable == 0, "chargeOff() знімає enable, а не лише опускає уставку");
    }

    printf("\n28б) плата TL494: вимір робить ПАКЕТ, і свіжість показань видно\n");
    {
        g_chg = ChargeState{};
        // а) що поклали — те й читаємо, обома функціями.
        uint16_t peak = 12345;
        chargeFeedPackReading(8100, 900);
        check(chargePackMv() == 8100, "напруга — та, що прийшла з DS2438");
        check(chargeMeasureMa(&peak) == 900, "струм — той, що прийшов з DS2438");
        check(peak == 0, "ВЕРШИНИ ПУЛЬСАЦІЙ немає, і це нуль, а не вигадка");

        // б) від'ємний струм (пакет розряджається) читається як «заряду немає».
        //    Випрямити його тут означало б сказати регуляторові «уставка вже
        //    досягнута» рівно тоді, коли струму бракує найбільше.
        chargeFeedPackReading(8100, -400);
        check(chargeMeasureMa(nullptr) == 0, "від'ємний струм — це нуль заряду, а не 400 мА");
        uint16_t up = chargeNextDuty(1000, chargeMeasureMa(nullptr), 800);
        check(up > 1000, "…і регулятор на нього ПІДНІМАЄ уставку");

        // в) регулятор на цій шкалі ходить РІВНО кроком CHARGE_TL_STEP_MV —
        //    хай яка похибка. Пропорційний крок тут заборонено фізично
        //    (див. коментар у settings.h), тож перевіряємо саме сталість.
        check(chargeNextDuty(1000, 0, 5000) - 1000 == CHARGE_TL_STEP_MV,
              "величезна похибка -> той самий крок");
        check(chargeNextDuty(1000, 0, 1000 + CHARGE_DEADBAND_MA + 1) - 1000 == CHARGE_TL_STEP_MV,
              "ледь помітна похибка -> той самий крок");
        check(chargeNextDuty(1000, 900, 900) == 1000, "у мертвій зоні не рухаємось");

        // г) і збіжність усе одно є — просто повільна.
        uint16_t duty = 0;
        for (int i = 0; i < 4000; i++) {
            // Модель: струм пропорційний перевищенню уставки над напругою
            // пакета (8.0 В) на внутрішньому опорі 0.4 Ом.
            int32_t ma = ((int32_t)duty - 8000) * 1000 / 400;
            if (ma < 0) ma = 0;
            duty = chargeNextDuty(duty, ma, 1000);
        }
        int32_t settled = ((int32_t)duty - 8000) * 1000 / 400;
        printf("   уставка збіглась на %u мВ -> %ld мА (ціль 1000)\n", duty, (long)settled);
        check(labs(settled - 1000) <= CHARGE_DEADBAND_MA + CHARGE_TL_STEP_MV * 1000 / 400,
              "контур виходить на уставку й тримається біля неї");
    }

    printf("\n28в) плата TL494: чого тут немає — сказано ЧЕСНО\n");
    {
        chargeSetOffByUser(false);
        // а) залізо описане — заряд доступний. Це не «версія без заряду».
        check(chargeHardware(),  "силова частина описана");
        check(chargeAvailable(), "заряд доступний");

        // б) власного виміру немає, і жодна з трьох ознак не вдає, ніби є.
        check(!chargePsuSensed(),      "контролю живлення +14 В немає");
        check(chargePsuState() == PSU_UNKNOWN, "…і стан його — «не знаю»");
        check(!chargePsuFault(),       "…тож і скарг на нього бути не може");
        check(!chargeSenseSaturated(65535), "свідка насичення клеми немає — подільника немає");
        uint16_t peak = 777;
        chargeFeedPackReading(8000, 500);
        chargeMeasureMa(&peak);
        check(peak == 0, "вершини пульсацій немає — шунта немає");

        // в) пробудження заборонено, і заборона НЕ залежить від решти умов:
        //    хай усе інше буде ідеальним, відповідь та сама.
        check(chargeWakeRefuse(true, true, false, PSU_OK, false, 0, 0) == WAKENO_TOPO,
              "пробудження заборонено навіть коли все інше гаразд");
        check(chargeWakeRefuse(false, false, true, PSU_ABSENT, true, 9000, 5000) == WAKENO_TOPO,
              "…і жодна інша причина її не перебиває");

        // г) причина названа СЛОВАМИ, і це слова про залізо, а не про settings.h.
        const char *why = chargeWakeRefuseText(WAKENO_TOPO);
        check(why[0] != '\0', "причина відмови не порожня");
        check(strstr(why, "DS2438") != nullptr, "…і пояснює, звідки беруться виміри");
        check(strstr(why, "settings.h") == nullptr,
              "не радимо правити settings.h там, де правити його нема чого");

        // ґ) і причина недоступності заряду називає ПІНИ ЦІЄЇ плати.
        chargeSetOffByUser(true);
        const char *nz = chargeUnavailText();
        check(strstr(nz, "версія") != nullptr, "вимикач дає причину про версію пристрою");
        chargeSetOffByUser(false);
        g_chg = ChargeState{};
    }

    printf("\n28г) плата TL494: моделі понижувача мовчать, а не вигадують\n");
    {
        // Три функції, які на понижувачі рахують дросель і живлення. Тут за
        // ними немає фізики — і саме тому вони мусять повертати НУЛЬ, а не
        // число, порахуване за чужими константами.
        check(chargeStartDuty(8000, 1000) == 0,
              "стартова уставка — нуль: soft-start, жодних оцінок наосліп");
        check(chargeStartDuty(6000, 200) == 0, "…і це не залежить від пакета");
        check(chargeBoundaryMa(8250) == 0, "межі неперервного режиму тут не існує");
        check(chargeBoundaryMa(6000) == 0, "…на жодній напрузі");
    }
