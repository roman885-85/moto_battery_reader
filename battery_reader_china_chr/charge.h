...
enum {
    ...
    // ── друга мета пробудження: пакет ЧИТАЄТЬСЯ, але клема мертва ──────────
    CHGR_WAKE_RAIL,   // клема ожила — захист пакета відпустив, ЦЕ УСПІХ
    CHGR_WAKE_NORAIL, // час вичерпано, а клема так і мертва: або захист не
                      // відпускає, або обірвано саму вимірювальну лінію
};
...
struct ChargeState {
    ...
    // Чого чекає сеанс: WAKE_GOAL_CHIP (монітор мовчить) чи WAKE_GOAL_RAIL
    // (монітор говорить, а клема мертва). Ставиться РАЗ на старті й далі не
    // перераховується: у меті RAIL монітор відповідає кожної проби, тож
    // «перевизначати мету на ходу» означало б щоразу оголошувати перемогу.
    uint8_t  wakeGoal;
...
// ── ДВІ МЕТИ ОДНОГО РЕЖИМУ ─────────────────────────────────────────────────
//  Пробудження показує заблокованому контролеру пакета «напругу зарядника»,
//  щоб той відпустив клеми. Але заблокованим він буває ПО-РІЗНОМУ, і ознака
//  успіху від цього залежить:
//
//   • WAKE_GOAL_CHIP — мовчить УСЕ: ні DS2438, ні DS2433. Успіх = монітор
//     заговорив. Це первісний випадок, заради якого режим і писався.
//
//   • WAKE_GOAL_RAIL — монітор ГОВОРИТЬ, а на клемі нуль. Тобто 1-Wire живий
//     (він сидить до силового ключа пакета), а силовий ключ захисту
//     розімкнутий. Успіх = на клемі з'явилась напруга.
//
//  ⚑ ДРУГИЙ ВИПАДОК РАНІШЕ БУВ ГЛУХИМ КУТОМ. Пробудження відмовляло, бо пакет
//  читається; штатний заряд відмовляв, бо на клемі нуль («подільник показує
//  майже нуль»). Обидві відмови окремо були розумні, а разом не лишали
//  користувачеві жодного шляху — при тому, що потрібна дія в обох випадках та
//  сама, і саме її пристрій уміє.
enum {
    WAKE_GOAL_CHIP = 0,   // монітор мовчить -> чекаємо, доки заговорить
    WAKE_GOAL_RAIL,       // монітор говорить, клема мертва -> чекаємо напруги
};

// «На клемі щось є». Той самий поріг, за яким штатний заряд вирішує, що
// подільник бачить пакет, — навмисно ОДИН на обидва рішення: два різні пороги
// дали б стан, у якому заряд каже «нуль», а пробудження каже «вже є напруга»,
// і жоден із режимів не запустився б.
inline bool chargeRailAlive(uint16_t mv) {
    return mv >= (uint16_t)(BATTERY_EMPTY_MV / 2);
}

// Мета сеансу за станом на момент старту.
inline uint8_t chargeWakeGoal(bool chipReads) {
    return chipReads ? WAKE_GOAL_RAIL : WAKE_GOAL_CHIP;
}
...
struct ChargeWakeIn {
    ...
    uint8_t  goal;       // WAKE_GOAL_* — чого саме чекаємо
    bool     tempFresh;  // температуру виміряли цього проходу
    int16_t  tempC10;    // і ось вона, у десятих градуса
};

enum {
    WAKEV_GO = 0,     // нічого не сталося — тримаємо напругу далі
    WAKEV_WOKE,       // монітор відповів: мета CHIP досягнута
    WAKEV_RAIL,       // на клемі з'явилась напруга: мета RAIL досягнута
    WAKEV_OVERI,      // середній струм пішов угору — відмова регулятора
    WAKEV_OVERV,      // на клемі напруга вище дозволеної
    WAKEV_HOT,        // пакет гарячий (лише мета RAIL — там є температура)
...
inline uint8_t chargeWakeVerdict(const ChargeWakeIn &in) {
    // ── УСПІХ. У двох метах він РІЗНИЙ, і сплутати їх не можна: у меті RAIL
    //  монітор відповідає з першої ж проби, тож перевірка «chipOk» оголосила б
    //  перемогу, не зробивши нічого.
    if (in.goal == WAKE_GOAL_CHIP) {
        if (in.chipOk) return WAKEV_WOKE;
    } else {
        // Насичений відлік — не напруга пакета, а стеля АЦП (див. нижче), і
        // зарахувати його як «клема ожила» означало б оголосити успіх на
        // обірваному колі.
        if (in.mvFresh && !chargeSenseSaturated(in.mv) && chargeRailAlive(in.mv))
            return WAKEV_RAIL;
    }
    if (in.avgMa > CHARGE_WAKE_MA_ABORT) return WAKEV_OVERI;
    // ⚑ ТЕМПЕРАТУРА — ЛИШЕ В МЕТІ RAIL, І ЦЕ НЕ ПОБЛАЖЛИВІСТЬ ДО ІНШОЇ. У меті
    //  CHIP монітор мовчить, тобто датчика фізично немає, і весь захист там
    //  тримається на межах енергії та часу. А в меті RAIL монітор ГОВОРИТЬ —
    //  не скористатись його температурою означало б лишити режим сліпим там,
    //  де він може бути зрячим.
    //
    //  Нижньої межі тут навмисно НЕМАЄ. 8 мА·год у пакет на кілька тисяч — це
    //  не заряд, а сигнал контролеру; холоду він не шкодить, а відмова на
    //  морозі просто лишила б людину без єдиного способу оживити пакет. Гріти
    //  ж гарячий пакет підстав немає ніколи.
    if (in.goal == WAKE_GOAL_RAIL && in.tempFresh &&
        in.tempC10 >= (int16_t)(CHARGE_MAX_TEMP_C * 10))
        return WAKEV_HOT;
...
// ⚑ МЕТА ВПЛИВАЄ НА ПЕРЕКЛАД ЛИШЕ ОДНОГО ВИРОКУ — «час вичерпано». І це не
//  дрібниця: у меті CHIP це «монітор так і мовчить», а в меті RAIL монітор
//  говорив увесь час, і невдача означає зовсім інше — або захист не відпустив,
//  або обірвано саму вимірювальну лінію. Один текст на два випадки відправив
//  би людину шукати не там.
inline uint8_t chargeWakeReason(uint8_t verdict, uint8_t goal) {
    switch (verdict) {
        case WAKEV_WOKE:    return CHGR_WOKE;
        case WAKEV_RAIL:    return CHGR_WAKE_RAIL;
        case WAKEV_OVERI:   return CHGR_WAKE_OVERI;
        case WAKEV_OVERV:   return CHGR_HARD_MAX;
        case WAKEV_HOT:     return CHGR_TEMP;
        case WAKEV_MAH:     return CHGR_WAKE_MAH;
        case WAKEV_TIMEOUT: return (goal == WAKE_GOAL_RAIL) ? CHGR_WAKE_NORAIL
                                                            : CHGR_WAKE_FAIL;
...
//  лише її половина — «пакет читається». Пробудження працює без контролю
//  температури, і
//  єдине, що не дає перетворити його на «заряд без датчика», — відмова
//  запускатись там, де доступний штатний заряд. Ключове слово ДОСТУПНИЙ:
//  «пакет читається» саме по собі цього не означає.
//
//  Заблокований пакет читається (1-Wire сидить ДО силового ключа захисту), а
//  на клемі в нього нуль — і штатний заряд на такому не стартує взагалі,
//  бо не бачить напруги. Стара умова відхиляла пробудження, штатний заряд
//  відхиляв себе сам, і користувач лишався без жодного шляху. Тому питання
//  тепер ставиться так, як його й треба було ставити: чи є ЧИМ замінити
...
inline uint8_t chargeWakeRefuse(bool available, bool pwmOk, bool busy, uint8_t psu,
    ...
    if (chipReads && chargeRailAlive(mv)) return WAKENO_READS;
    ...
    // WAKENO_FULL питає лише мету CHIP — але жодного «!chipReads» тут не
    // треба, і додати його було б гірше, ніж не додати.
    //
    // ⚑ ЦЕ ВИХОДИТЬ САМО. До цього рядка з живим монітором ми доходимо лише
    //  тоді, коли клема МЕРТВА (інакше вище спрацював би WAKENO_READS), тобто
    //  mv < BATTERY_EMPTY_MV/2 = 3000 мВ. А поріг тут — CHARGE_WAKE_MV =
    //  8000 мВ. Умова «мертва клема і водночас вище 8 В» суперечлива, тож
    //  зайвий доданок ніколи б не змінив відповіді — а саме такі доданки в
    //  цьому проєкті вже тричі виявлялись мертвими перевірками, у які всі
    //  вірили. Замість доданка — перевірка співвідношення двох порогів
...
// сама домовленість, що й у фаз профілю (chargePhaseText/chargePhaseShort).
//
// ⚑ ТЕКСТ ЗАЛЕЖИТЬ ВІД МЕТИ, і це не косметика: у меті RAIL монітор говорить
//  із самого початку, тож напис «монітор мовчить» був би просто неправдою —
//  причому саме в той момент, коли людина за ним і стежить.
inline const char *chargeWakeGoalText(uint8_t goal, uint8_t reason) {
    if (goal == WAKE_GOAL_RAIL)
        return (reason == CHGR_WAKE_RAIL) ? "КЛЕМА ОЖИЛА" : "клема ще мертва";
    return (reason == CHGR_WOKE) ? "ВІДПОВІВ" : "монітор мовчить";
}
inline const char *chargeWakeGoalShort(uint8_t goal, uint8_t reason) {
    if (goal == WAKE_GOAL_RAIL)
        return (reason == CHGR_WAKE_RAIL) ? "КЛЕМА!" : "нема В";
    return (reason == CHGR_WOKE) ? "ВІДПОВІВ!" : "мовчить";
}
...
inline const char *chargeWakeRefuseText(uint8_t r) {
    switch (r) {
        ...
        case WAKENO_READS: return "Пакет читається І на клемі є напруга — пробудження "
                                  "не потрібне. Воно працює без контролю температури, "
                                  "тож дозволене лише там, де штатний заряд неможливий; "
                                  "тут можливий — користуйтесь ним, він безпечніший";
...
inline const char *chargeReasonText(uint8_t r) {
    switch (r) {
        ...
        case CHGR_WAKE_RAIL:return "пакет ожив — захист відпустив клеми";
        case CHGR_WAKE_NORAIL:
            return "клема так і лишилась мертвою: або захист не відпускає, "
                   "або обірвано лінію CHARGE_VSENSE_PIN";
