...
int main() {
    ...
    //  ніхто — і заряд ніколи не оновлював екран.
    {
        ...
        // Перевіряємо НАСЛІДОК, а не спосіб: у режимі пробудження плитка
        // температури мусить показувати прочерк. Через який саме віджет —
        // байдуже (панель уже переписували), а от «0.0 °C» замість прочерку
        // означало б видати відсутність даних за дані.
        check(fileHasText("usb_client/moto_gui.py", "v[\"T\"] = \"—\""),
