...
int main() {
    ...
    printf("\n45) жести: сходинки не плутаються, ланцюжок не набирається сам\n");
    {
        ...
        //  жесту — ті самі, якими гортають меню; людина натискає їх сотнями за
        //  сеанс. Повноекранний напис, що вискакує посеред роботи, читається як
        ...
        const uint8_t L = CKEY_LEFT, R = CKEY_RIGHT, E = CKEY_ENTER;

        // а) правильний ланцюжок спрацьовує — і рівно на останній клавіші.
        {
            Seq q;
            check(!q.key(L) && !q.key(R) && !q.key(R) && !q.key(L),
                  "перші чотири клавіші мовчать");
            check(q.key(E), "…і жест спрацьовує рівно на п'ятій");
            check(!q.key(E) && !q.key(E), "…а після спрацювання лічильник порожній");
        }
        ...
        {
            Seq q;
            q.key(L); q.key(R); q.key(R); q.key(L);
            check(!q.key(E, COMBO_SEQ_WINDOW_MS + 1),
                  "не встиг у вікно — жест не зараховано");
            // ⚑ І ЛІЧИЛЬНИК ПІСЛЯ ЦЬОГО ПОРОЖНІЙ, А НЕ «МАЙЖЕ ПОВНИЙ».
            //  Інакше одна клавіша через півдня добирала б учорашній жест.
            check(!q.key(E), "…і добрати його однією клавішею неможливо");
        }
        {
            // Той самий набір, але вкладений у вікно, — спрацьовує.
            Seq q;
            check(!q.key(L, 900) && !q.key(R, 900) && !q.key(R, 900) && !q.key(L, 900) &&
                  q.key(E, 900), "неспішний набір у межах 5 с зараховується");
        }
        {
            //  ⚑ ВІКНО РАХУЄТЬСЯ ВІД ПЕРШОЇ КЛАВІШІ, А НЕ ВІД ПОПЕРЕДНЬОЇ.
            //  Різниця видна саме тут: паузи по 2 с кожна прийнятні для
            //  «допуску темпу», але в сумі виходять за вікно.
            Seq q;
            q.key(L, 100); q.key(R, 2000); q.key(R, 2000);
            check(!q.key(L, 2000) && !q.key(E, 100),
                  "сума пауз теж рахується: вікно на ВЕСЬ жест, а не між клавішами");
        }
        {
            ...
            check(!q.key(R) && !q.key(L) && !q.key(E),
                  "лічильник гасне сам собою, а не «на наступному натисканні»");
        }

        // в) ОБМОВКА НА ПОЧАТКУ НЕ ЛАМАЄ ЖЕСТ. Зайве перше «‹» — найзвичайніша
        //    з них, і без відкату жест не спрацював би НІКОЛИ.
        {
            Seq q;
            check(!q.key(L) && !q.key(L) && !q.key(R) && !q.key(R) && !q.key(L) && q.key(E),
                  "зайве перше «‹» не заважає: набір починається заново з нього");
        }
        {
            //  І складніший випадок: майже повний жест, у якому замість «OK»
            //  натиснули «‹» — це «‹» і стає початком нового набору.
            Seq q;
            q.key(L); q.key(R); q.key(R); q.key(L);
            check(!q.key(L), "замість «OK» натиснули «‹» — жесту немає");
            check(!q.key(R) && !q.key(R) && !q.key(L) && q.key(E),
                  "…але саме воно стало початком, і жест добирається");
        }

        // г) ⚑ ЖЕСТ НЕ НАБИРАЄТЬСЯ ЗВИЧАЙНОЮ РОБОТОЮ. Перебираємо все, що
        //    людина справді робить у меню: гортання в один бік, гортання
        //    туди-сюди, вхід у пункт, вихід. Жодна з цих послідовностей не
        //    сміє показати прихований напис.
        {
            const uint8_t WALK_FWD[]  = { R, R, R, R, R, R, R, R, E };
            const uint8_t WALK_BACK[] = { L, L, L, L, L, L, L, L, E };
            const uint8_t WOBBLE[]    = { R, L, R, L, R, L, R, L, E };
            const uint8_t ENTER_EXIT[]= { R, E, L, E, R, E, L, E, R };
            struct { const uint8_t *v; int n; const char *what; } DAY[] = {
                { WALK_FWD,   9, "гортання вперед і вибір" },
                { WALK_BACK,  9, "гортання назад і вибір" },
                { WOBBLE,     9, "гортання туди-сюди" },
                { ENTER_EXIT, 9, "вхід-вихід по пунктах" },
            };
            for (auto &d : DAY) {
                Seq q; bool fired = false;
                for (int rep = 0; rep < 20; rep++)
                    for (int i = 0; i < d.n; i++)
                        if (q.key(d.v[i], 300)) fired = true;
                char m[120];
        ...
        {
            //  ⚑ І НАЙЖОРСТКІШЕ: ВИПАДКОВИЙ НАБІР. Тисяча натискань поспіль із
            //  тих самих трьох клавіш. Жест, який трапляється на випадковому
            //  наборі частіше за кілька разів, у житті вискакував би сам.
            Seq q; int fired = 0;
            uint32_t rnd = 12345;
            for (int i = 0; i < 1000; i++) {
                rnd = rnd * 1103515245u + 12345u;
                uint8_t k = (uint8_t)(1 + ((rnd >> 16) % 3));
                if (q.key(k, 300)) fired++;
            }
            char m[120];
            snprintf(m, sizeof(m), "тисяча випадкових натискань дала жест %d раз(и)", fired);
            check(fired <= 6, m);
        }

        // д) скидання забирає лічильник цілком.
        {
            Seq q; q.key(L); q.key(R); q.key(R); q.key(L);
            comboSeqReset(q.s);
            check(!q.key(E), "після скидання жест не добирається однією клавішею");
        }
        ...
        // е) «нема клавіші» — це не клавіша.
        {
            ...
            check(!q.key(L) && !q.key(R) && !q.key(R) && !q.key(L) && q.key(E),
                  "…і не заважає справжньому набору");
        }
