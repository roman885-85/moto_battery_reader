...
#else
// ⚑ ЗАПАСНА ГІЛКА МУСИТЬ ОГОЛОШУВАТИ ТЕ САМЕ, ЩО Й ОСНОВНА. Доти тут бракувало
//  btName() — і рівно через це «використано раніше, ніж оголошено» неможливо
//  було впіймати препроцесором: у хостовому проході цієї функції не існувало
//  взагалі, тож перевірці не було що звіряти. А в Arduino IDE вона існувала —
//  просто пізніше за того, хто її кличе, і збірка падала вже в власника.
//
//  Тип повернення тут інший (const char* замість String) навмисно: String —
//  це Arduino, а ця гілка живе саме там, де Arduino немає. Обидві однаково
//  придатні для того єдиного, що з ними роблять, — додавання до JSON.
inline bool btUp()        { return false; }
inline bool btConnected() { return false; }
inline void btBegin()     {}
inline const char *btName() { return ""; }
#endif  // BT_ENABLED
