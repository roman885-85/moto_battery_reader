...
int main() {
    ...
    printf("\n1) струм рахується від ЄМНОСТІ ПАКЕТА, але не вище заліза\n");
    {
        ...
        // ⚑ ЧИСЛА ТУТ ЗАЛЕЖАТЬ ВІД ЗАЛІЗА, А ПРАВИЛО — НІ. Стеля профілю
        //  (CHARGE_MA_80) у двох пристроїв різна: 1000 мА там, де силовий ключ
        //  наш і його тепловий бюджет рахує прошивка, і 1500 мА на готовій
        //  платі TL494, де ключ розпаяв виробник. Тому перевіряємо саме
        //  ПРАВИЛО «менше з двох», а затиск доводимо окремо — на пакеті,
        //  якому 0.5C завідомо більше за будь-яку зі стель.
        //  Раніше тут стояло «== CHARGE_MA_80» для пакета 2500, і на другому
        //  пристрої це перетворювалось на вимогу зарядити 2500 мА·год струмом
        //  1500 мА замість чесних 1250 — тобто перевірка вимагала помилки.
        const uint32_t half2500 = 2500UL * CHARGE_SMART_CC_PCT / 100;
        check(chargeSmartCMa(2500, CHARGE_SMART_CC_PCT) ==
                  (half2500 < (uint32_t)CHARGE_MA_80 ? (uint16_t)half2500
                                                     : (uint16_t)CHARGE_MA_80),
              "пакету 2500 дістається МЕНШЕ З ДВОХ: 0.5C і апаратна стеля");
        const uint16_t huge =
            (uint16_t)((uint32_t)CHARGE_MA_80 * 100 / CHARGE_SMART_CC_PCT + 1000);
        printf("   стеля профілю %u мА -> затиск пробуємо на пакеті %u мА·год\n",
               (unsigned)CHARGE_MA_80, huge);
        check(chargeSmartCMa(huge, CHARGE_SMART_CC_PCT) == CHARGE_MA_80,
    ...
    printf("\n2) фази йдуть у правильному порядку за напругою\n");
    {
        ...
        // Знову ПРАВИЛО, а не число: струм фази CC — це той самий «менше з
        // двох», і брати сюди голу стелю означало б повторити ту саму помилку.
        check(smart(7500, TGT, 2500).setMa == chargeSmartCMa(2500, CHARGE_SMART_CC_PCT),
              "…повним дозволеним для ЦЬОГО пакета струмом");
