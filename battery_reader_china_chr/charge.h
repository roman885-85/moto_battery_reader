...
//
//  ── ЩО ЦЕ ЗА СХЕМА ────────────────────────────────────────────────────────
//  P-канальний MOSFET у верхньому ключі, після нього — діод на землю й
//  дросель на вихід: класичний понижувач. Ключем керує ОДИН ШІМ-пін через
//  NPN; два інвертування (NPN + P-канал) взаємно скасовуються, тож логіка
//  пряма: шпаруватість на піні = шпаруватість ключа, 0 % = закрито = безпечно.
//
//  ── ГОЛОВНЕ, ЩО ТРЕБА ТРИМАТИ В ГОЛОВІ ────────────────────────────────────
//
//  1. Uвих ≈ D × Uживл. Струм у пакет з'являється ЛИШЕ коли D перевищить
//     Uпакета/Uживл (≈0.6 при 8.4 В від 14 В). Нижче — струму немає взагалі.
//     Тому soft-start «з нуля крок за кроком» тут БЕЗГЛУЗДИЙ: до робочої
//     точки довелося б повзти сотні кроків (при кроці 2 з 2047 — понад
//     десять хвилин). Замість цього старт іде з РОЗРАХОВАНОЇ шпаруватості
//     chargeDutyForMv(Uпакета) — це точка нульового струму, тобто так само
//     безпечно, але одразу; далі регулятор долає лише невелику різницю.
//
...
//
//  3. Частота ШІМ висока (десятки кГц) — цього вимагає дросель. Наслідок:
//     затворне коло мусить встигати (τ = R_pullup × Ciss), інакше ключ
//     гріється в лінійному режимі. Перевірка на компіляції в settings.h.
//
...
//  виміряний середній струм з уставкою профілю й зсуваємо шпаруватість на
//  CHARGE_DUTY_STEP у потрібний бік. Старт — із chargeDutyForMv() (нульовий
//  струм), стеля — CHARGE_DUTY_MAX_PCT.
//
//  ── БЕЗПЕКА ────────────────────────────────────────────────────────────────
...
//
// ⚑ Навіщо. У понижувача струм у пакет з'являється лише коли D перевищить
// Uпакета/Uживл — до того виходу просто немає. Стартувати з нуля й повзти
// кроками CHARGE_DUTY_STEP означало б хвилини «мертвого» розгону (при 2 з
// 2047 і секундному опитуванні — понад десять хвилин). Тому старт іде звідси:
// це РІВНО точка нульового струму — так само безпечно, як нуль, але одразу
// біля робочої зони, і далі регулятор долає лише невелику різницю.
//
// Це навмисно груба оцінка (ідеальний перетворювач, без урахування падінь на
// ключі, дроселі, діоді й шунті) — саме тому вона дає струм НЕ БІЛЬШИЙ за
// нуль, а не менший: усі знехтувані падіння лише зменшують реальний вихід.
...
inline void chargeInit() {
    ...
    Serial.printf("CHARGE: buck L=%d мкГн, f=%d Гц -> пульсації ~%ld мА; "
                  "стеля D=%d%% дає ~%ld мВ; затвор τ=%ld нс (період %ld нс)\n",
                  (int)CHARGE_L_UH, (int)CHARGE_PWM_FREQ,
                  (long)CHARGE_RIPPLE_MA_EST,
                  (int)CHARGE_DUTY_MAX_PCT,
                  (long)((long)CHARGE_SUPPLY_MV * CHARGE_DUTY_MAX_PCT / 100),
                  (long)((long)CHARGE_GATE_PULLUP_OHM * CHARGE_GATE_CISS_PF / 1000),
                  (long)(1000000000L / CHARGE_PWM_FREQ));
#endif
