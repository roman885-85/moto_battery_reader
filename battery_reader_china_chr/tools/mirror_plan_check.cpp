...
int main() {
    ...
    printf("\nX) наробіток узгоджується з датою, записаною в пакеті\n");
    {
        ...
        check(!restoreEtmFixPlan(p, &etm, &why) && strstr(why, "сьогоднішньої дати"),
              "без сьогоднішньої дати крок відмовляється — і каже саме це");
        p.todayY = 2026; p.todayM = 8; p.todayD = 21;
        check(!restoreEtmFixPlan(p, &etm, &why) && strstr(why, "дати першого запуску"),
        ...
        check(restoreEtmFixPlan(p, &etm, &why), "розходження бачить і береться правити");
        ...
        p.etmPack = 0L;
        check(!restoreEtmFixPlan(p, &etm, &why), "порожній наробіток крок не «дораховує»");
        p.etmPack = 12L * 3600L;                       // пів доби проти доби
        check(!restoreEtmFixPlan(p, &etm, &why), "наробіток МЕНШИЙ за вік крок не чіпає");

        // Доба допуску: різниця в години — округлення, а не розходження.
        p.etmPack = 1L * 86400L + 3600L;
        check(!restoreEtmFixPlan(p, &etm, &why), "різниця в години не вважається розходженням");
        p.etmPack = 3L * 86400L;
        check(restoreEtmFixPlan(p, &etm, &why), "…а різниця в кілька діб — вважається");

        // Дата, вписана вручну, теж підходить як джерело.
        p.useReal = 0; p.useUserY = 2026; p.useUserM = 8; p.useUserD = 20;
        check(restoreEtmFixPlan(p, &etm, &why), "вписана вручну дата теж годиться");
