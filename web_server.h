...
// ⚑ bt_link.h — САМЕ ТУТ, А НЕ «ВІН І ТАК ПРИЙДЕ ЗІ СКЕТЧА». Ім'я приладу в
//  ефірі (btName) віддає й ця відповідь теж, а сам заголовок доти включав лише
//  serial_api.h — і той стоїть у скетчі ПІСЛЯ web_server.h. Тобто складалось
//  усе рівно доти, доки звідси ніхто по btName() не звертався; щойно
//  звернувся — «btName was not declared in this scope» просто в Arduino IDE.
//  Заголовок захищений #pragma once, тож зайвим включенням нічого не коштує.
#include "bt_link.h"          // btName(): ім'я, під яким прилад видно в ефірі
