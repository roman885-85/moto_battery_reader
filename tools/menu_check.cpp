...
int main() {
    ...
    printf("\n13) зв'язок: п'ятий рядок списку, і зникнути йому нема з чим\n");
    {
        ...
        //  через клієнта, а кнопками. Кожен зайвий рядок тут — це зайве
        //  натискання наосліп.
        int idx = menuIndexOfOp(OP_RADIO);
        char m[120];
        snprintf(m, sizeof(m), "«Зв'язок» стоїть %d-м рядком (треба 5-м)", idx + 1);
        check(idx == 4, m);
