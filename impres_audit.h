...
enum {
    ...
    AUD_DCA_INSANE    = 1u << 7,  // з пакета «взято» в рази більше, ніж залито
};
...
//  44 доби.
//  ⚑ «ВІК НЕВІДОМИЙ» ПОЗНАЧАЄТЬСЯ ВІД'ЄМНИМ, А НЕ НУЛЕМ. Нуль діб — це
//  ЗАКОННИЙ вік: пакет, якому щойно вписали сьогоднішню дату виготовлення
//  (а так і роблять під час ремонту), існує рівно нуль діб. Стара умова
//  `ageDays > 0` мовчки оголошувала такий пакет «нема з чим порівнювати» —
//  і тисячі діб у моніторі, тобто найгрубіше з можливих розходжень, не
//  помічались узагалі. Три клієнти при цьому рахували те саме правило без
//  цієї умови й попередження показували: у Майстра кроку не було, а на
//  картці даних попередження було — саме так виглядає «прилад скаржиться,
//  а полагодити не пропонує».
...
inline uint32_t impresAudit(const uint8_t *d33, const uint8_t *d38,
    ...
    // ⚑ ЛІЧИЛЬНИК РОЗРЯДУ — ДО перевірки ключа, бо ключа він не потребує:
    //  CCA й DCA лежать у моніторі відкритим текстом. Поставити цю перевірку
    //  нижче означало б мовчати про побитий регістр на кожному пакеті, ключ
    //  якого не визначився, — тобто саме там, де діагностика найпотрібніша.
    if (d38 && !impresBmsDcaSane(b.cca, b.dca)) f |= AUD_DCA_INSANE;
