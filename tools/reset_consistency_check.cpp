...
int main() {
    ...
    // ── 10. ТРЕТЯ МІТКА ПОДІЇ — В ДОБАХ, І ЇЇ ТЕЖ ТРЕБА ОБНУЛЯТИ ───────────
    //  Скарга власника: «при чистому DS2433 обнулив лічильник наробітку
    //  монітора, вставив у зарядку, витягнув — стало 4545». DS2433 був чистий,
    //  ETM і обидві 32-бітні мітки — нулі. Взятись числу було нізвідки, крім
    //  четвертого місця, якого ми не знали: двобайтового поля за 0x32, де та
    //  сама подія записана в ДОБАХ.
    //
    //  ⚑ ЦЕ НЕ ЗДОГАД. У корпусі є дамп, який доводить це прямо:
    //  dumps/19-stantsiya-dobudova/01_2438 — ETM 0, мітка1 0, мітка2 0, CCA 0,
    //  тобто скидання бездоганне за всім, що ми знали, — і 4015 діб у 0x32.
    printf("\n10) третя мітка (доби) не переживає скидання монітора\n");
    {
        int seen = 0, hadStamp = 0, survived = 0, perfect = 0;
        for (auto &p : files) {
            std::string p38 = pair38(p);
            if (!load(p38.c_str(), d38, DS2438_MEM_SIZE)) continue;
            seen++;
            uint16_t before = impresStampDays(d38);
            if (before) hadStamp++;
            // ⚑ ДАМПИ, ДЕ СКИДАННЯ ВЖЕ ЗРОБЛЕНО «ІДЕАЛЬНО» за старими
            //  правилами, — і мітка все одно жива. Саме вони й пояснюють
            //  скаргу, тож рахуємо їх окремо.
            if (!impresEtm(d38) && !impres38U32(d38, IMPRES_38_STAMP1_AT) &&
                !impres38U32(d38, IMPRES_38_STAMP2_AT) && before) perfect++;

            uint8_t w38[DS2438_MEM_SIZE];
            memcpy(w38, d38, DS2438_MEM_SIZE);
            impresResetMonitor(w38, nullptr, 0);
            if (impresStampDays(w38)) survived++;
        }
        printf("   моніторів %d: із третьою міткою %d, пережили скидання %d; "
               "дампів «ідеально скинуто, а мітка жива» %d\n",
               seen, hadStamp, survived, perfect);
        check(seen > 40, "корпус зчитався");
        // Без цього рядка перевірка нижче доводила б порожнечу.
        check(hadStamp > 20, "третя мітка справді трапляється — є що обнуляти");
        check(survived == 0, "після скидання монітора третьої мітки не лишається");
        check(perfect > 0, "…і в корпусі є дамп, де вона пережила бездоганне скидання");

        // Правка наробітку теж мусить її підтягувати: лишити мітку більшою за
        // наробіток — це той самий неможливий стан «подія в майбутньому».
        uint8_t t38[DS2438_MEM_SIZE];
        memset(t38, 0, sizeof(t38));
        t38[IMPRES_38_STAMPD_AT]     = 0xC1;      // 4545 діб — число власника
        t38[IMPRES_38_STAMPD_AT + 1] = 0x11;
        impresSetEtm(t38, 100UL * 86400UL);       // ставимо 100 діб
        check(impresStampDays(t38) == 100, "правка наробітку підтягує й третю мітку");
        impresSetEtm(t38, 200UL * 86400UL);       // а ось ПІДІЙМАТИ її не треба
        check(impresStampDays(t38) == 100,
              "…але не піднімає: подія не могла статись пізніше, ніж була");
    }
